...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        if (user?.verified) {
            return NextResponse.json({ success: false, error: "Email already verified" }, { status: 400, headers: corsHeaders });
        }

        user.verified = true;
        await user.save();
        return NextResponse.json({ success: true, message: "Email verified successfully" }, { status: 200, headers: corsHeaders });
        ...
        if (error?.name === "TokenExpiredError") {
            return NextResponse.json({ success: false, error: "Token has expired" }, { status: 401, headers: corsHeaders });
        }
